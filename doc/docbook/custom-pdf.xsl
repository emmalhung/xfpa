...
<xsl:param name="latex.class.options">a4paper,12pt,twoside,openright</xsl:param>
<xsl:param name='latex.hyperparam'>colorlinks,linkcolor=darkblue</xsl:param>	<!-- Make links blue -->
...
<!-- Allow bold words to be split between lines
<xsl:template name="inline.boldmonoseq">
  <xsl:param name="content">
    <xsl:apply-templates/>
  </xsl:param>
...
<xsl:template name="inline.italicmonoseq">
  ...
  <xsl:text>\texttt{\emph{\textbf{\small{</xsl:text>
  <xsl:choose>
  <xsl:when test="$monoseq.hyphenation='0'">
    <xsl:text>&lt;h&gt;</xsl:text>
    <xsl:copy-of select="$content"/>
    <xsl:text>&lt;/h&gt;</xsl:text>
  </xsl:when>
