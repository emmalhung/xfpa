/***********************************************************************
*                                                                      *
*     e x a m p l e _ p l o t . c                                      *
*                                                                      *
*     Template for creating "Scattered" type metafiles from point data *
*                                                                      *
*   Usage:  example_plot  <setup_file>  (<optional_config_info>)       *
...
STRING	argv[];
	...
	if ( argc < (numargs + 1) )
		...
		(void) fprintf(stderr, "  example_plot  <setup_file>");
	...
	/* Set field descriptor for plot data */
	if ( !set_fld_descript(&fdesc,
	...
	/* Create METAFILE Object to hold PLOT data */
	meta = create_metafile();
	(void) define_mf_tstamp(meta, fdesc.rtime, fdesc.vtime);
	(void) define_mf_projection(meta, mproj);

	/* Create PLOT Object to hold data */
	splot = create_plot();
	(void) add_subfld_to_plot(splot, "LABELS",   "label", NULL);
	(void) add_subfld_to_plot(splot, "WINDS",    "barb",  NULL);
	(void) add_subfld_to_plot(splot, "INTEGERS", "int",   NULL);
	(void) add_subfld_to_plot(splot, "FLOATS",   "float", NULL);

	/* Set PLOT Object data for each location */
	for ( iloc=0; iloc<NumLctn; iloc++ )
		...
		(void) add_point_to_plot(splot, pos);
		(void) define_subfld_value(splot, "LABELS",
								iloc, label, label, 0,    0,    0,    0);
		(void) define_subfld_value(splot, "WINDS",
								iloc, NULL,  NULL,  0,    0,    drct, spd);
		(void) define_subfld_value(splot, "INTEGERS",
								iloc, NULL,  NULL,  ival, ival, 0,    0);
		(void) define_subfld_value(splot, "FLOATS",
								iloc, NULL,  NULL,  0,    0,    fval, fval);
		}

	/* Move PLOT Object to METAFILE Object */
	(void) add_plot_to_metafile(meta, "d", fdesc.edef->name, fdesc.ldef->name,
			splot);
	...
	/* Output METAFILE Object containing PLOT Object */
